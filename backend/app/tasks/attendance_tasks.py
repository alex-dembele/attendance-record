...
from app.models.attendance import AttendanceEntry, AttendanceRawImport
...
def process_attendance_file(self, import_id: str, file_path: str):
    """
    Tâche Celery pour traiter le fichier de présence uploadé.
    Cette version est plus robuste pour la manipulation des dates et heures.
    Elle déclenche ensuite le calcul des sessions de travail.
    """
    db = get_db_session()
    raw_import = (
        db.query(AttendanceRawImport)
        .filter(AttendanceRawImport.id == import_id)
        .first()
    )
    if not raw_import:
        db.close()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if not employee:
                logger.warning(
                    f"Ligne {index}: Employé avec l'ID {row['employee_id']} non trouvé. Ligne ignorée."
                )
                continue
            ...
            if pd.notna(row["entry_time"]):
                try:
                    ...
                    logger.warning(
                        f"Ligne {index}: Impossible de traiter l'heure d'entrée '{row['entry_time']}'. Erreur: {e}"
                    )
            ...
            if pd.notna(row["exit_time"]):
                try:
                    ...
                    logger.warning(
                        f"Ligne {index}: Impossible de traiter l'heure de sortie '{row['exit_time']}'. Erreur: {e}"
                    )
        ...
        if entries_created > 0:
            ...
            calculate_work_sessions_task.delay(
                affected_employee_ids, start_date_str, end_date_str
            )
...
@celery_app.task
def calculate_work_sessions_task(
    employee_ids: list[str], start_date: str, end_date: str
):
    """Tâche de fond pour calculer les sessions de travail."""
    logger.info(
        f"Début du calcul des sessions de travail pour {len(employee_ids)} employés, du {start_date} au {end_date}."
    )
    db = get_db_session()
    try:
